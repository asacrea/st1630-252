...
producer = KafkaProducer(
    bootstrap_servers=[KAFKA_BOOTSTRAP],
    # La key es un string simple (el nombre de la región). Si alguna
    # vez fuera None -- mensaje sin key, como los de la Parte 0.5 --
    # hay que dejarla pasar como None y NO intentar .encode() sobre
    # ella: None le dice a Kafka "reparte con el partitioner por
    # defecto (sticky)" en vez de "hashea esta key".
    key_serializer=lambda k: k.encode("utf-8") if k is not None else None,
    # ensure_ascii=False para que "Bogotá" viaje como UTF-8 real y no
    # como una secuencia escapada tipo backslash-u dentro del JSON.
...
def enviar_pedido(pedido: dict):
    ...
    # key=region: el partitioner hace murmur2(region) % 4, así que la
    # misma región cae SIEMPRE en la misma partición -> orden garantizado
    # por región, a costa de balanceo (Bogotá = 40% del tráfico).
    future = producer.send(TOPIC, key=pedido["region"], value=pedido)
    # Envío SÍNCRONO: .get() bloquea hasta que el líder responde el ack
    # (con acks="all", hasta que todos los ISR confirmaron). Si el envío
    # falla, .get() levanta la excepción aquí mismo en vez de perderla
    # silenciosamente en un callback.
