...
consumer = KafkaConsumer(
    ...
    group_id=GROUP_ID,
    auto_offset_reset="earliest",
    # NO auto-commit: el offset lo movemos nosotros, y solo después de
    # que el MERGE a Bronze terminó bien (ver el loop de main()).
    enable_auto_commit=False,
    value_deserializer=lambda v: json.loads(v.decode("utf-8")),
    key_deserializer=lambda k: k.decode("utf-8") if k is not None else None,
    # Único parámetro que agregamos por fuera de lo que pide el TODO, y
    # lo agregamos porque el pipeline no funciona sin él: cada MERGE
    # Delta tarda ~2 s, y con el default (max_poll_records=500) el
    # consumidor pasa ~15 min procesando un lote sin volver a llamar a
    # poll(). El broker lo da por muerto al superar
    # max_poll_interval_ms (5 min por defecto), lo saca del grupo, y a
    # partir de ahí TODO consumer.commit() falla con CommitFailedError
    # -- el MERGE sigue funcionando, pero el offset ya no avanza nunca.
    # Con lotes de 10, poll() vuelve a llamarse cada ~20 s.
    # (Evidencia de las dos veces que nos pasó: datos/log_consumidor_run4_drenaje.txt
    #  y datos/log_consumidor_run5_drenaje.txt.)
    max_poll_records=10,
)
...
def main():
    ...
    for mensaje in consumer:
        try:
            fila = construir_fila_bronze(mensaje)
            merge_a_bronze(fila)
            # El commit va DESPUÉS del MERGE y solo si el MERGE no lanzó:
            # si el proceso muere entre el MERGE y esta línea, Kafka
            # vuelve a entregar el mensaje (at-least-once) y el MERGE por
            # pedido_id lo absorbe sin duplicar -> Bronze queda igual.
            consumer.commit()
            contador_procesados += 1
            print(f"[OK] offset={mensaje.offset} partition={mensaje.partition} "
                  f"pedido_id={fila['pedido_id']}", flush=True)
        except Exception as e:
            # Sin commit: el offset del grupo se queda atrás y este mismo
            # mensaje se reprocesa en el próximo arranque del consumidor.
            contador_rechazados += 1
            print(f"[ERROR] offset={mensaje.offset} partition={mensaje.partition} "
