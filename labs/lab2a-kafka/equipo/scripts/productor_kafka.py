...
"pedidos-ventas". Entrega del equipo: los bloques 1.1,
1.3 y 1.4 que el script traía marcados como TODO están implementados
abajo. El generador de datos y la configuración de conexión venían
dados. Las dos decisiones de esta semana -- qué key usar y qué nivel
de acks pedir -- se justifican en ../kafka_design.md (Preguntas 1 y 2).
...
# ═══════════════════════════════════════════════════════════════
# 1.1 · Configuración del KafkaProducer
# ═══════════════════════════════════════════════════════════════
...
#
# Configuración aplicada, y el porqué de cada parámetro:
...
producer = KafkaProducer(
    bootstrap_servers=[KAFKA_BOOTSTRAP],
    # La key es un string (el nombre de la región) o None. El `if k is not
    # None` no es defensivo por gusto: kafka-python llama al serializer
    # también cuando la key es None, y None.encode() reventaría. Que la key
    # pueda ser None es justo el caso de la Parte 0, pregunta 5 -- sin key,
    # el particionador no puede hashear nada y reparte por otro criterio.
    key_serializer=lambda k: k.encode("utf-8") if k is not None else None,
    # ensure_ascii=False para que "Bogotá" viaje como UTF-8 real y no como
    # la secuencia escapada "Bogot\u00e1" -- el consumidor hace
    # json.loads sobre estos bytes y escribe el string a Delta tal cual.
...
#
# Envío síncrono implementado abajo:
#   1. future = producer.send(TOPIC, key=pedido["region"], value=pedido)
#   2. metadata = future.get(timeout=10)  # bloquea hasta la confirmación
#   3. metadata.partition y metadata.offset alimentan el log y el conteo
def enviar_pedido(pedido: dict):
    """Envía un pedido y devuelve (partition, offset) para logging."""
    # key=pedido["region"]: el particionador por defecto calcula
    # murmur2(key) % n_particiones, así que una misma región siempre cae en
    # la misma partición -- eso es lo que da el orden por región.
    future = producer.send(TOPIC, key=pedido["region"], value=pedido)
    # future.get() bloquea hasta que el líder (y, con acks="all", todas las
    # réplicas ISR) confirman la escritura. Es lo que hace este envío
    # SÍNCRONO: cada mensaje espera su confirmación antes de mandar el
    # siguiente. Cuesta throughput y es exactamente el trade-off que mide
    # la variante asíncrona de la sección 1.5.
...
def main():
    ...
    for i in range(N_PEDIDOS):
        ...
        # 1.4 · Se acumula SIEMPRE (en los 1.000 mensajes), pero solo se
        # imprime cada 100 para no inundar la terminal. El conteo completo
        # es la evidencia de la Pregunta 2 de kafka_design.md.
