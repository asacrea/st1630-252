...
| 8 | `SparkSession` no puede leer/escribir Delta (`Failed to find the data source: delta`) | Falta el paquete `delta-spark` instalado en tu entorno Python | `pip install pyspark delta-spark` — el script ya llama a `configure_spark_with_delta_pip` internamente (usa la versión de `delta-spark` que tengas instalada), no necesitas `--packages` a mano ni `PYSPARK_SUBMIT_ARGS` |
