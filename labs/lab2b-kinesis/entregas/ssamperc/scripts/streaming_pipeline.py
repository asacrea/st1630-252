...
def aplicar_ventana(df: DataFrame) -> DataFrame:
    return (
        df.withWatermark("kafka_time", "3 minutes")
        .groupBy(
            F.window(F.col("kafka_time"), "2 minutes"),
            F.col("region"),
        )
        .agg(
            F.sum("total").alias("ventas_totales"),
...
def escribir_batch(df_micro_batch: DataFrame, id_batch: int) -> None:
    ...
    tabla_existente = DeltaTable.forPath(spark, SILVER_STREAMING_PATH)
    (
        tabla_existente.alias("existente")
        .merge(
