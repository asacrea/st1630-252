...
def aplicar_ventana(df: DataFrame) -> DataFrame:
    # Ventana de 5 min y watermark de 10 min: decisión del equipo,
    # justificada en streaming_design.md (Pregunta 1).
    return (
...
def escribir_batch(df_micro_batch: DataFrame, id_batch: int) -> None:
    # El DataFrame de foreachBatch se recalcula en cada acción; se cachea
    # porque aquí se usa dos veces (el conteo del log y la escritura).
    df_micro_batch.persist()
    filas = df_micro_batch.count()

    if filas == 0:
        # Batch sin datos (Spark los lanza para avanzar el watermark):
        # no hay nada que mergear, y así no se crean commits vacíos en Delta.
        print(f"[batch {id_batch}] sin cambios", flush=True)
    elif not DeltaTable.isDeltaTable(spark, SILVER_STREAMING_PATH):
        df_micro_batch.write.format("delta").mode("overwrite").save(SILVER_STREAMING_PATH)
        print(f"[batch {id_batch}] {filas} filas ventana+región -> tabla creada", flush=True)
    else:
        (
        ...
        print(f"[batch {id_batch}] {filas} filas ventana+región -> MERGE", flush=True)
