...
# Cambio del equipo (Parte 4): KINESIS_ENDPOINT permite apuntar a LocalStack
# (http://localstack:4566), porque AWS Academy no nos da kinesis:CreateStream.
# Sin la variable, el conector usa el endpoint real de AWS de KINESIS_REGION.
KINESIS_ENDPOINT = os.environ.get("KINESIS_ENDPOINT")
# El conector que usa format("aws-kinesis") es el de AWS Labs, que no está en
# Maven Central: se descarga como JAR (ver ../README.md) y se carga con spark.jars.
KINESIS_CONNECTOR_JAR = os.environ.get(
    "KINESIS_CONNECTOR_JAR", "/jars/spark-streaming-sql-kinesis-connector_2.12-1.4.2.jar"
)
...
if STREAM_SOURCE == "kinesis":
    _builder = _builder.config("spark.jars", KINESIS_CONNECTOR_JAR)
...
def crear_stream_kafka(spark) -> DataFrame:
    return (
        ...
        .load()
        .select(
            # value llega como bytes: cast a string y parseo con el mismo
            # schema que publica productor_kafka.py. Si un mensaje no
            # cumple el schema, from_json devuelve null en vez de fallar.
            F.from_json(F.col("value").cast("string"), PEDIDO_SCHEMA).alias("d"),
            # timestamp es el CreateTime del mensaje (lo fija el productor
            # al enviarlo): es el tiempo de evento para las ventanas, no
            # la hora en que Spark lo procesa.
            F.col("timestamp").alias("kafka_time"),
        )
        .select("d.*", "kafka_time")
    )
...
#
# Nota del equipo: ese paquete de Qubole es para Spark 3.0 y registra el
# formato "kinesis", no "aws-kinesis". Las opciones kinesis.* de abajo son
# del conector de AWS Labs (spark-sql-kinesis-connector), que cargamos como
# JAR arriba. La única línea agregada a esta función es kinesis.endpointUrl.
def crear_stream_kinesis(spark) -> DataFrame:
    ...
    if KINESIS_ENDPOINT:
        reader = reader.option("kinesis.endpointUrl", KINESIS_ENDPOINT)
    return (
        reader
        .load()
        .select(
...
def aplicar_ventana(df: DataFrame) -> DataFrame:
    # Decisión del equipo: ventana de 5 minutos, watermark de 10 minutos.
    # Justificación en streaming_design.md, Pregunta 1.
    return (
        df
        # withWatermark va ANTES del groupBy: sin él, Spark no puede
        # cerrar ventanas y el estado crece sin límite.
...
def escribir_batch(df_micro_batch: DataFrame, id_batch: int) -> None:
    filas = df_micro_batch.count()
    print(f"[batch {id_batch}] {filas} filas de ventana actualizadas")
    if filas == 0:
        return

    if not DeltaTable.isDeltaTable(spark, SILVER_STREAMING_PATH):
        # Primera vez: la tabla no existe todavía, no hay contra qué comparar.
        df_micro_batch.write.format("delta").mode("overwrite").save(SILVER_STREAMING_PATH)
        return

    # Llave = (window_start, window_end, region): si Spark vuelve a entregar
    # la misma ventana+región con más pedidos, la fila se pisa (UPDATE) con
    # el agregado más reciente; con append quedaría duplicada.
    (
        DeltaTable.forPath(spark, SILVER_STREAMING_PATH).alias("existente")
        .merge(
